...
const Login = () => {
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      // ✅ Admin -> /admin, Client -> /sites
      navigate(user.role === 'admin' ? '/admin' : '/sites', { replace: true });
    }
  ...
  const submit = async (event) => {
    ...
    setFormError('');
    if (!email || !password) {
      return setFormError('Enter your email and password to continue.');
    }
    ...
    // ✅ Admin -> /admin, Client -> /sites
    navigate(result.user?.role === 'admin' ? '/admin' : '/sites', { replace: true });
  };
      ...
      <div className="portal-shell auth-shell">
        ...
        <aside className="portal-sidebar auth-brand">
          ...
          <div className="auth-features">
            <p><FiShield /> Secure project access</p>
            <p><FiUser /> Client-specific dashboards</p>
            <p><FiLock /> Protected media & deliverables</p>
          </div>
          <div className="portal-client">
            <span>Precision in Data.</span>
            <strong>Perfection in Visualization.</strong>
            <small>© {new Date().getFullYear()} Aeroview 360</small>
          </div>
        </aside>
        ...
        <main className="portal-main auth-main">
          <form onSubmit={submit} className="auth-form">
            <div className="portal-heading__avatar mx-auto">
              <FiUser />
            </div>
            <h1>Welcome Back!</h1>
            ...
            <p className="auth-help">
              Your account is created by an Aeroview administrator.
            </p>
          </form>
        </main>
      </div>
